...
- [~] 8g. Final code review [x] (no blockers; listener-cap warning fixed) · security review SC5 = NOT READY (2 High: runner acts on unverified reconnects [PoC]; quests inherit user allow rules and all tools; 3 Med: home config writable, no timeout, save temp file follows symlinks) → fixes [~] · QA end-to-end incl. real-CLI quest/receptionist and runner acceptance (a)–(i) → fixes → v0.3.0
