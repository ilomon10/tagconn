...
export const SECTION_LABELS: Record<keyof Settings, { title: string; hint: string }> = {
  ...
  office: { title: 'Office', hint: 'Look and feel of the pixel office. Applies as soon as you save, no restart needed.' },
...
export const ENUM_OPTIONS: Record<string, readonly string[]> = {
  ...
  'office.shaders.vignetteStyle': ['pixel', 'smooth'],
  'office.shaders.screen': ['off', 'crt', 'lcd', 'vhs'],
...
export const KEY_HINTS: Record<string, string> = {
  ...
  'office.shaders.vignette': 'Vignette — darkness at the very screen edge; 0 = off. The middle of the screen always stays clean.',
  'office.shaders.vignetteSize': 'Vignette width — how far it reaches in from the edges, as a fraction of the shorter screen side (0.12 = 12%).',
  'office.shaders.vignetteStyle': 'Vignette style — pixel: hard stepped bands on a blocky pixel-art grid (default). smooth: a soft fade along the edges.',
  'office.shaders.vignetteSteps': 'Vignette shade bands for the pixel style (4 = 25/50/75/100% of the vignette strength).',
  'office.shaders.vignettePixel': "Vignette pixel style block size, in art pixels — scales with the camera zoom so blocks stay aligned with the art's own pixels.",
  ...
  'office.shaders.scanlines': "Legacy CRT look on the modern style, used only while Screen is off — prefer Screen: CRT. Kept for old configs.",
  'office.shaders.screen': "Screen — the default monitor effect for every viewer (off/CRT/LCD/VHS); each browser's top bar can override it (docs/decisions.md #25).",
  'office.shaders.screenStrength': 'Screen effect intensity; 0 = off.',
